...
# Define case structure for handling packets by ID, with id included
macro generate_server_packet_handler(list)
  case state
  {% for current_state in list %}
  when {{current_state[0]}}
    case packet.id
    ...
    when {{data[2]}}
      {{data[1].id}} = {{data[0]}}.new(packet)
      self.handle(id, {{data[1].id}})
      @emitter.emit.{{data[1].id}}(id, {{data[1].id}})
    {% end %}
    else
      Log.warn { "Unknown packet ID from #{id}: #{"0x%02x" % packet.id} (length: #{packet.data.size})" }
    end
  {% end %}
  end
end
...
macro generate_server_handler(definitions, emitter)
  {% handshaking_packets = definitions[0] %}
  {% status_packets = definitions[1] %}
  {% login_packets = definitions[2] %}
  {% play_packets = definitions[3] %}
  ...
  # Define the handler for raw packet data
  def handle(id : String, state : ProtocolState, data : Bytes)
    packet = @parser.parse(data)

    generate_server_packet_handler([
      {ProtocolState::Handshaking, {{handshaking_packets}}},
      {ProtocolState::Status, {{status_packets}}},
      {ProtocolState::Login, {{login_packets}}},
      {ProtocolState::Play, {{play_packets}}},
  ...
  {% for packets in definitions %}
    {% for packet in packets %}
      def handle(id : String, packet : {{packet[0]}})
      end
    {% end %}
  {% end %}
end
